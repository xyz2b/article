<!-- ════════════════════════════════════════════════════════════════
  写作纪律（写之前先看，别删，发布前再删）
  1. 深度红线（用户上一轮 AskUserQuestion 拍板）：本篇 = 缺页异常内核怎么一步步处理，**命名路径级**。
     点到 do_page_fault / find_vma / handle_mm_fault / handle_pte_fault / do_anonymous_page / do_wp_page / do_swap_page
     这些函数名 + 它们的先后顺序 + 每一步在判什么，但【绝不贴内核源码、不做结构体(vm_area_struct/pte_t)字段走读】。
     读者不查内核也能跟上，认真的能拿函数名去 grep 源码。
  2. 一个 aha 不综述。主钉子 = 「内核接手后，是一棵决策树：先问这地址合不合法(有没有VMA)，再问这页该从哪来(清零/磁盘/复制)」。
     所有真实数据都为这棵树的某个分支作证，不堆料。
  3. 图是主角，纯 ASCII，中文标注放框外。核心是一张【决策树总图】，其余是各分支的小图。
  4. 写完别打磨第七遍。发出去＝完成。
  5. 协作：机械讲解 Claude 写，带体温的 4 处（钩子/那张决策树的恍然/minor vs major那个咦/收尾）用户用自己声音填。
  6. 真实产物：全部 docker 真跑（python:3.11-slim 容器, aarch64, kernel 6.12, 4KB页）。命令在正文，复制能复现。改数前回 /lab 重跑。
  7. 衔接：上一篇《页表多级+TLB》结尾勾了两个悬念=swap + 「缺页内核到底怎么一步步处理」。本篇吃掉后者；swap 在本篇作为「do_swap_page 分支」被点到但不展开(它自己还能再写一篇)。

  标题候选（用户选一个，删掉其余）：
  A. 一次缺页，内核接手后到底干了什么——跟着 page fault 走一遍它的决策树
  B. 缺页中断不是“出错”，是内核最忙的一条正常路径
  C. 从 *p 那一下到内核补上一页：缺页异常的完整处理路径
═══════════════════════════════════════════════════════════════ -->

# 缺页中断不是“出错”，是内核最忙的一条正常路径

<!-- 用户填：钩子。
  建议接住前两篇攒下的认知：我们已经知道地址是假的(虚拟)、按页翻译、查多级页表、TLB兜底。
  也已经知道"翻不出来就触发缺页中断"——但上两篇都只把缺页当一个黑箱结论：要么段错误，要么内核补一页。
  这篇要把这个黑箱打开：内核接手之后，到底一步步做了什么判断、按什么顺序、最后怎么补上那一页(或者决定杀了你)。
  你可以用自己的话点一个反直觉的点：缺页(page fault)名字里带 fault，听着像出错，
  但其实你程序里绝大多数缺页是【完全正常】的——malloc 完第一次写、程序第一次跑到某段代码、读一个 mmap 的文件，
  全靠缺页才落地。它是内核最高频的正常工作，不是异常。
  下面这段是我垫的，按你的声音改： -->
...
<!-- 用户填：那张决策树的恍然。
  这里是全文骨架第一次成形的地方。点一句：
  原来"段错误"不是一种错，是两种——一种是地址压根不存在(没VMA)，
  一种是地址在、但你越权(有VMA权限不符)。内核第一步干的全是"安检"，
  跟"去哪弄内存"还没半点关系。它先确认你有没有资格，再谈给不给。
  用你自己的话说说这个"哦原来分这两层"。下面是我垫的： -->
...
<!-- 用户填：minor vs major 那个咦。
  这是第二个 aha 的落点。点一句这组对照狠在哪：
  同样是"第一次碰 16384 个页、触发 16384 次缺页"，一个 major=0、一个 major=16384，
  天壤之别——区别不在你写的代码(都是循环逐页访问)，而在内核第二步判断"这页该从哪来"时，
  匿名内存走了"凭空造"、文件页走了"去磁盘取"。
  你平时说的"程序卡了一下"，很可能就是 major fault：内核在替你等磁盘。
  minor 是微秒级、major 是毫秒级，差三个数量级。
  用你自己的话说说这个区分为什么值得记住(调性能时 major fault 高就是信号)。下面是我垫的： -->
...
<!-- 用户填：收尾。这是你声音最该出现的地方。
  可以收束到这一条主线：缺页处理是一棵决策树，两层判断——
  第一层安检(有没有VMA / 权限符不符)，没过就是你熟悉的段错误；
  第二层是"这页从哪来"(凭空造 / 去磁盘 / 从swap换回 / 复制)，对应你程序里几乎所有正常的内存落地。
  补完页、回到原指令重跑，整个过程对你的代码完全透明——
  你写的每一句 malloc 后的赋值、每一次读文件、fork 后的每一次写，背后都在悄悄走这棵树。
  可以再点一句:把前三篇连起来——地址是假的(虚拟)→按页查多级页表→翻不动就缺页→内核走这棵树补上→重跑指令翻译通过。
  这四篇合起来,就是"你打印的那个地址，怎么一步步变成内存条上真实一格"的完整故事。
  用你自己的话收。下面是我垫的骨架： -->
...
<!-- 发布前自检（删之前过一遍）：
  □ 真实数字没手滑改错（段错误139 / 匿名写16384 minor / 匿名读32 / 文件读16384 major / COW 4096 / time -v 那个16468）——拿不准回 docker /lab 重跑
  □ 「这些缺页数你怎么自己看到」那个小节的命令真能跑：/usr/bin/time -v(写全路径!) 输出 Minor/Major page faults；getrusage 前后相减；/proc/<pid>/stat 第10/12字段
  □ 函数名拼写核对（do_page_fault / find_vma / handle_mm_fault / handle_pte_fault / do_anonymous_page / filemap_fault / do_swap_page / do_wp_page）
  □ 4 处「用户填」都用自己的声音填了，AI 文风清掉了
  □ ASCII 图在掘金编辑器贴预览，箭头框线没跑偏；重点查第六节那张【完整决策树总图】(最宽)
  □ 没手痒去展开 swap/页面回收(那是下一篇)、没去贴内核源码或 vm_area_struct 字段(用户定的是命名路径级，不是源码走读)
  □ 发布！连成第六次，守住习惯。
-->
